...
describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-11-17'))).toBe('2024년 11월 3주');
  });
  ...
  it('겹치는 주차에 대해서는, 기준이 되는 월에 대한 주차를 반환한다.', () => {
    // 겹치는 경우에 목요일을 가지는 월이 기준이 된다.
    expect(formatWeek(new Date('2024-11-02'))).toBe('2024년 10월 5주');
    expect(formatWeek(new Date('2024-10-31'))).toBe('2024년 10월 5주');
  });
...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    expect(isDateInRange(new Date('2024-07-05'), rangeEnd, rangeStart)).toBe(false);
  });
});
...
describe('fillZero', () => {
  ...
  test("소수점이 있는 3.14를 5자리로 변환하면 '00003.14'를 반환한다", () => {
    expect(fillZero(3.14, 5)).toBe('00003.14');
  });
  ...
  test('value가 지정된 size보다 큰 자릿수를 가지면 원래 값을 그대로 반환한다', () => {
    expect(fillZero(1234522, 5)).toBe('1234522');
  });
});

describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    expect(formatDate(new Date('2024-11-5'))).toBe('2024-11-05');
  });

  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    expect(formatDate(new Date('2024-11-5'), 10)).toBe('2024-11-10');
  });

  it('월이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    expect(formatDate(new Date('2024-9'), 5)).toBe('2024-09-05');
  });
